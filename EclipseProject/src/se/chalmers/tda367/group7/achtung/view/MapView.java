...
public class MapView implements View {
	...
	public void render(RenderService renderer, float interpolation) {
		...
		renderer.setViewAreaSize(this.map.getWidth() + 200, this.map.getHeight());
		renderer.setBackgroundColor(Color.DARK_GRAY);

		renderer.drawFilledRect(0, 0, this.map.getWidth(), this.map.getHeight(),
				this.map.getColor());

		// Draws line around the world
		renderer.drawLinedRect(0, 0, this.map.getWidth(), this.map.getHeight(), 5,
				Color.GRAY);
	}
}
